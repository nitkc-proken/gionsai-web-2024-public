...
export function loadEmoji(code: string, type: keyof typeof apis = "twemoji") {
	const key = `${type}:${code}`;
	if (key in emojiCache) return emojiCache[key];

	const api = apis[type];
	if (typeof api === "function") {
		const result = fetch(api(code)).then((r) => r.text());
		emojiCache[key] = result;
		return result;
	}
	const result = fetch(`${api}${code.toUpperCase()}.svg`).then((r) => r.text());
	emojiCache[key] = result;
	return result;
}
